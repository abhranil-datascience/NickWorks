...
import os,random,datetime,fitz,io,sys,requests,string,json
...
class PerformOCRGVA(Resource):
    def post(self):
        ...
        if file_url.endswith('.pdf'):
            ...
            return{'msg':'Error','description':'Invalid file. Only pdf/jpg/jpeg/png files are supported. Please ensure that the url contains a valid Extension'}
        ...
        ################ Read File and Change to jpeg format ####################
        ...
        if not(IsValid):
            ...
            try:
                ...
                return {'msg':'Error','description':'Unable to extract information from Google Vision API response.'}
###################### Update Records in database #####################################
class UpdateRecordsinDB(Resource):
    def post(self):
        ...
        if "Surname" in data.keys():
            ...
            Surname="NA"
        if "GivenName" in data.keys():
            GivenName=data['GivenName']
        else:
            GivenName="NA"
        if "PlaceOfDeath" in data.keys():
            PlaceOfDeath=data['PlaceOfDeath']
        else:
            PlaceOfDeath="NA"
        if "DateOfDeath" in data.keys():
        ...
        if "Age" in data.keys():
            ...
            Age="NA"
        if "DateOfBirth" in data.keys():
            DateOfBirth=data['DateOfBirth']
        else:
            DateOfBirth="NA"
        if "UsualAddress" in data.keys():
            UsualAddress=data['UsualAddress']
        else:
            UsualAddress="NA"
        if "PlaceOfBirth" in data.keys():
            PlaceOfBirth=data['PlaceOfBirth']
        else:
            PlaceOfBirth="NA"
        #################### Check If All the fields are null ##################################
        if ((Surname=="NA") and (GivenName=="NA") and (PlaceOfDeath=="NA") and (DateOfDeath=="NA") and (Occupation=="NA") and (Sex=="NA") and
            (Age=="NA") and (DateOfBirth=="NA") and (UsualAddress=="NA") and (PlaceOfBirth=="NA")):
            return {'msg':'Error','description':'Mandatory fields are null.'}
        else:
            ####################### Insert Data into Database #############################
            url = "https://deathcertificateocr-0847.restdb.io/rest/ocr-values"
            data_dict={'Surname':Surname,'Given Name':GivenName,'Place Of Death':PlaceOfDeath,'Date Of Death':DateOfDeath,'Occupation':Occupation,
                       'Sex':Sex,'Age':Age,'Date Of Birth':DateOfBirth,'Usual Address':UsualAddress,'Place Of Birth':PlaceOfBirth}
            data_json=json.dumps(data_dict)
            DBKey=os.getenv("DBKey")
            headers = {'content-type': "application/json",'x-apikey': DBKey,'cache-control': "no-cache"}
            try:
                response = requests.request("POST", url, data=data_json, headers=headers)
            except:
                return {'msg':'Error','description':'Unable to call rest DB API.'}
            if response.status_code==201:
                ResponseID=json.loads(response.text)['_id']
                return {'msg':'Success','description':'Record inserted successfully.','DocumentID':ResponseID}
            else:
                return {'msg':'Error','description':'Unable to insert record in database.'}
#################### Configure URLs #########################
api.add_resource(PerformOCRGVA,'/PerformOCRGVA')
api.add_resource(UpdateRecordsinDB,'/UpdateRecordsInDB')
#################  Run Flask Server ##########################
